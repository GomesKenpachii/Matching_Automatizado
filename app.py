...
# --- Novos imports para pré-processamento ---
import unidecode
import re
import nltk
...
# --- Configuração do NLTK (executado apenas uma vez, se necessário) ---
@st.cache_resource
def setup_nltk():
    """Baixa os recursos do NLTK (stopwords) se ainda não estiverem presentes."""
    try:
...
# Use um set para buscas mais rápidas
stop_words_pt = set(stopwords.words('portuguese'))

def limpar_texto(texto):
    """Aplica uma série de limpezas a uma string de texto."""
    if not isinstance(texto, str):
        return ""
    texto = texto.lower()  # 1. Minúsculas
    texto = unidecode.unidecode(texto)  # 2. Remove acentos
    texto = re.sub(r'[^a-z\s]', '', texto)  # 3. Remove pontuação e números
    # 4. Remove stopwords
...
# --- Funções de Carregamento de Dados (AGORA COM LIMPEZA) ---
@st.cache_data
def carregar_e_preparar_jovens(arquivo_jovens):
    """Carrega, renomeia e LIMPA as colunas do dataframe de jovens."""
    try:
        bd_jovem = pd.read_csv(arquivo_jovens, sep=",", on_bad_lines="skip")
        # ... (renomeação de colunas permanece igual)
        bd_jovem = bd_jovem.rename(columns={
            'Nome Completo:': 'Nome Completo', 'Email:': 'Email Jovem', 'CPF:': 'CPF Jovem', 'Telefone:': 'Telefone Jovem',
...
def carregar_e_preparar_mentores(arquivo_mentores):
    """Carrega, renomeia e LIMPA as colunas do dataframe de mentores."""
    try:
        bd_mentor = pd.read_csv(arquivo_mentores)
        # ... (renomeação de colunas permanece igual)
        bd_mentor = bd_mentor.rename(columns={
            'Nome': 'Nome Mentor', 'Email': 'Email Mentor', 'CPF': 'CPF Mentor', 'Telefone': 'Telefone Mentor',
            'Qual o seu curso de formação?': 'Curso Mentor', 'Quais são suas áreas de atuação?': 'Área de atuação',
...
# --- Função Principal de Pareamento (com a barra de progresso corrigida) ---
def realizar_pareamento(jovens, mentores, peso_tfidf, peso_sbert, num_mentores_por_jovem, max_jovens_por_mentor, porcentagem_minima):
    # ... (O restante da função de pareamento continua exatamente o mesmo da versão anterior)
    ...
    if total_jovens == 0:
        st.warning("O arquivo de jovens está vazio. Nenhum pareamento a ser feito.")
        return pd.DataFrame()
    ...
    for contador, (i, jovem) in enumerate(jovens.iterrows()):
        ...
        for mentor_idx, score in similaridade_jovem:
            ...
            if (mentores_disponiveis.get(mentor_email, 0) < max_jovens_por_mentor and score * 100 >= porcentagem_minima and mentores_atribuidos < num_mentores_por_jovem):
                pareamentos.append({
                    'Nome Jovem': jovem.get('Nome Completo'), 'Email Jovem': jovem.get('Email Jovem'), 'CPF Jovem': jovem.get('CPF Jovem'), 'Telefone Jovem': jovem.get('Telefone Jovem'),
                    ...
                    'Nome Mentor': mentores.iloc[mentor_idx].get('Nome Mentor'), 'Email Mentor': mentor_email, 'Telefone Mentor': mentores.iloc[mentor_idx].get('Telefone Mentor'), 'CPF Mentor': mentores.iloc[mentor_idx].get('CPF Mentor'),
                    'Curso Mentor': mentores.iloc[mentor_idx].get('Curso Mentor'), 'Área de atuação': mentores.iloc[mentor_idx].get('Área de atuação'), 'Bio': mentores.iloc[mentor_idx].get('Bio'),
                ...
                if mentores_atribuidos >= num_mentores_por_jovem:
                    break
        progresso_atual = (contador + 1) / total_jovens
        barra_progresso.progress(progresso_atual, text=f"Pareando jovens... {contador+1}/{total_jovens}")
    time.sleep(0.5)
...
# --- Interface Gráfica com Streamlit (Inalterada) ---
...
if arquivo_jovens and arquivo_mentores:
    ...
    if bd_jovem is not None and bd_mentor is not None:
        st.success("Arquivos carregados e textos pré-processados com sucesso!")
        ...
        if st.button("🚀 Realizar Pareamento", type="primary"):
            with st.spinner("Aguarde, o processo pode levar alguns minutos..."):
                df_resultados = realizar_pareamento(
                    bd_jovem, bd_mentor, peso_tfidf, peso_sbert,
                    num_mentores_por_jovem, max_jovens_por_mentor, porcentagem_minima
                )
